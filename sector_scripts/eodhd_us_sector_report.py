...
def fetch_instrument_name(ticker: str) -> str:
    url = f"https://eodhd.com/api/search/{ticker}?api_token={EODHD_API_TOKEN}&fmt=json"
    ...
    if isinstance(data, list) and data:
        ...
        for row in data:
            candidate = f"{row.get('Code','')}.{row.get('Exchange','')}".upper()
            if candidate == ticker_u:
...
<head>
  ...
  <title>US Sector Performance Report</title>
  <style>
    :root {{
      --bg: #f5f7fb;
      --card: #ffffff;
...
<body>
  <div class="wrap">
    <div class="card">
      <h1>US Sector ETF Report</h1>
      <p>Data source: EODHD API (`adjusted_close`, dividend/split adjusted)</p>
...
def main() -> None:
    ...
    for name, ticker in SERIES:
        instrument_name = fetch_instrument_name(ticker)
    ...
    for col in rel.columns:
        lw = 2.8 if col in ("S&P 500", "Nasdaq 100") else 1.4
        alpha = 1.0 if col in ("S&P 500", "Nasdaq 100") else 0.85
        ax.plot(rel.index, rel[col], label=col, linewidth=lw, alpha=alpha)

    ax.set_title("US Sectors Relative Performance (Base=100, 1Y)", fontsize=13)
    ...
    chart_path = "output/us_sector_relative_performance.png"
    table_path = "output/us_sector_metrics.csv"
    map_path = "output/us_sector_mapping.csv"
    prices_path = "output/us_sector_prices.csv"
    html_path = "output/us_sector_report.html"
